...
The simulated observer programme exhibits a step-wise decrease in $CV_{BPUE}$, which increases with refusal rate. This is due to large refusal rate suppressing the realized vessel sampling. Large refusal rates mean that even though target proportion increases, the realised proportion doesn't change. Therefore, CV stays constant until the target proportion overcomes the refusal rate enough to add one more vessel. In addition, the small size of the fishery (31 vessels) magnifies the effect.
...
1.  **Separate spatial, temporal, and spatiotemporal effects in simulation models.** We extended the original SCOTI framework to represent situations where there is spatial variation in the probability of a bycatch event occurring (i.e., bycatch hotspot(s)). The bycatch hotspot(s) in this function can be switched on and off during specific periods in a year. We modified the fishery-simulating function in SCOTI to include multiple areas, one or more of which can be identified as a hotspot(s) (e.g., an area where seabirds might be actively feeding) and a temporal trend (e.g., an area with a breeding colony where there is a temporal effect on the probability of bycatch occurring). We also modified the function with the possibility to include temporal and spatial trends in fishery effort across the areas. To simulate the Norwegian case study, a single hotspot area for birds was active during the summer months (aggregation due to breeding season), and the fishery had a tendency to fish more intensively in the same area throughout the whole year. The inclusion of a spatiotemporal bycatch hotspots thus increased the overall BPUE in the simulation, compared to a simulation without a hotspot area with increased probability of bycatch. We did not have enough information in the real data to reliably reduce this trend by specifically parameterize how the BPUE is expected to change between hotspot areas and non-hotspot areas in the simulation. As the mean BPUE in this case study is a rather small number (close to zero), a rather small increase in the mean (for example from 0.02 to 0.03 birds per haul) will also change the CV to a large degree compared to changes in the SD. This is thus more an artifact of how the CV is calculated (sensitive to small changes in the mean when the mean is close to zero), rather than an effect of variations in fisheries or hotspot areas for this case study. In general, the CV is not a good measure of the precision of monitoring effort when the mean BPUE in the fishery is close to zero, which is often the case of bycatch of PET species.
2.  **Take advantage of existing tagging and monitoring data to generate realistic simulations.** The spatio temporal simulation extension in SCOTI needs input from both bird distribution data and fishery data in order to be parameterized to simulate realistic scenarios. Spatiotemporal species distribution models (e.g., VAST; sdmTMB) can be fitted with monitoring and tagging data and used to generate simulated distribution data that can be used in a simulation-estimation framework. Fishery data could be extracted from for example VMS data, to understand changes in fishery effort distribution throughout a year.
